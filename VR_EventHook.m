function Message = VR_EventHook( Message)
...
% Meel Velliste 8/11/2008
    ...
    switch(Message.msg_type)
        ...
        case 'APP_ERROR'
            ModIDNames = fieldnames( RTMA.MID);
            ModIDNums = struct2cell( RTMA.MID);
	...
	switch(M.msg_type)
        case 'PS3_BUTTON_PRESS'
           if(M.data.controllerId == XM.config.use_controller)
                if(M.data.whichButton == RTMA.defines.PS3_B_X)
                  disp('X recd');
                  if(isfield(XM, 'runtime'))
                    XM.runtime.cancel_button_pressed = 1;
                  end
                  M.msg_type = 'PROCEED_TO_Failure';
                elseif(M.data.whichButton == RTMA.defines.PS3_B_R1)
                  disp('R1 recd');
                  M.msg_type = 'PROCEED_TO_NextState';
                elseif(M.data.whichButton == RTMA.defines.PS3_B_R2)
                  disp('R2 recd');
                  M.msg_type = 'PROCEED_TO_NextState';
                elseif(M.data.whichButton == RTMA.defines.PS3_B_SQUARE)
                  disp('PS3 Square button received');
                ...
                elseif(M.data.whichButton == RTMA.defines.PS3_B_CIRCLE)
                  disp('circle recd');
                  GiveDirectReward(5);
                end 
           end
